defmodule Poller.PollServer do
  ...
  def handle_call({:add_question, question}, _from, state) do
    poll = Poll.add_question(state, question)
    {:reply, poll, poll}
  end

  def handle_call({:add_vote, choice_id}, _from, state) do
    poll = Poll.vote(state, choice_id)
    {:reply, poll, poll}
  end
